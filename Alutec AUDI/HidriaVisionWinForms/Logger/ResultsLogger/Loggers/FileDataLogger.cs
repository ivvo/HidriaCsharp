...
{
    public class FileDataLogger<T> : DataLoggerBase<T> where T : struct
    {
        #region Constants
        ...
        /// <summary>
        /// Constructs object of type FileDataLogger.
        /// </summary>
        ...
        public FileDataLogger(string folderPath, string station, int typeNum, int programNum, int maxNumOfEntries, Action<Task> errorHandlingCallback) : base(errorHandlingCallback)
        {
            ...
            while (!Data.IsCompleted)
            {
                ...
                if (base.Data.TryTake(out T Data, 50))
                {
                    ...
                    if (CurrentLogFile == null)
                    {
                        ...
                        NumOfEntries = File.ReadAllLines(CurrentLogFile.Value.FullLogFilePath).Length;
                    ...
                    foreach (FieldInfo structField in StructFields)
                        ResultLogEntry.AppendFormat(CultureInfo.InvariantCulture, "|{0} : {1}|", structField.Name, structField.GetValue(Data));
